...
struct Edge3D {
  Edge3D(std::shared_ptr<Vertex3D> a, std::shared_ptr<Vertex3D> b, Eigen::Vector3d p, Eigen::Quaterniond q,
         EdgeType type, Eigen::Matrix<double, 6, 6> information)
      : a_(a), b_(b), p_(p), q_(q), type_(type), information_(information){};
