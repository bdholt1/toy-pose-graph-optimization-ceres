...
void PoseGraph2D::AddNode(Node2D* node)
{
  nodes_.push_back(node);
}

void PoseGraph2D::AddEdge(Edge2D* edge)
{
...
void PoseGraph2D::LoadFromFile(const std::string& filename) {
  ...
  while( std::getline(fp, line) )
  {
    ...
    if( words[0].compare( "EDGE_SE2") == 0 )
    {
      ...
      Edge2D* edge = new Edge2D( nodes_[a_indx], nodes_[b_indx], indices_follow ? EdgeType::Odometry : EdgeType::LoopClosure );
